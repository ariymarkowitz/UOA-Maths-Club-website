        ...
        if (slide.classList.contains('stack') === false) {
            ...
            if (
                ...
                var slideNumberH =
                    ...
                    slideNumberV =
                        parseInt(slide.getAttribute('data-index-v'), 10) + 1;
            ...
            // Show all fragments
                ...
                toArray(
                    page.querySelectorAll('.fragment:not(.fade-out)')
...
function createBackgrounds() {
    ...
    toArray(dom.wrapper.querySelectorAll(HORIZONTAL_SLIDES_SELECTOR)).forEach(
        function(slideh) {
            ...
            toArray(slideh.querySelectorAll('section')).forEach(function(
                slidev
            ) {
        ...
        for (var i = 0; i < slidesLength; i++) {
            ...
            if (i < index) {
                ...
                if (config.fragments) {
                    ...
                    toArray(element.querySelectorAll('.fragment')).forEach(
                        function(fragment) {
                            fragment.classList.add('visible');
                            fragment.classList.remove('current-fragment');
                        }
...
function stopEmbeddedContent(element, options) {
    ...
    if (element && element.parentNode) {
        ...
        if (options.unloadIframes === true) {
            ...
            toArray(element.querySelectorAll('iframe[data-src]')).forEach(
                function(el) {
...
function getIndices(slide) {
    ...
    if (slide) {
        ...
        if (isVertical) {
            v = Math.max(
                toArray(slide.parentNode.querySelectorAll('section')).indexOf(
                    slide
                ),
                0
            );
    ...
    if (!slide && currentSlide) {
        ...
        if (hasFragments) {
            var currentFragment = currentSlide.querySelector(
                '.current-fragment'
            );
            if (
                currentFragment &&
                ...
                f =
                    currentSlide.querySelectorAll('.fragment.visible').length -
                    1;
            }
        }
    }
...
function updateFragments(index, fragments) {
    ...
    if (currentSlide && config.fragments) {
        fragments =
            fragments ||
            sortFragments(currentSlide.querySelectorAll('.fragment'));
